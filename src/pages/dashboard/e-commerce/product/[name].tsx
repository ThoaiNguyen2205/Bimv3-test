...
export default function EcommerceProductDetailsPage() {
  ...
  const { data, isLoading, checkout } = useSelector((state) => state.product);
  ...
  const TABS = [
    {
      value: 'description',
      label: 'description',
      component: data ? <Markdown children={data?.description} /> : null,
    },
    {
      value: 'reviews',
      label: `Reviews (${data ? data.reviews.length : ''})`,
      component: data ? <ProductDetailsReview product={data} /> : null,
    },
  ];
  ...
  return (
    <>
      <Head>
        <title>{`Ecommerce: ${data?.name || ''} | BIMNEXT V3`}</title>
      </Head>
